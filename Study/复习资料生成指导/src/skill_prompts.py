...
【纪律强化（必须逐条执行）】
1. **保真**：公式用 `$...$` / `$$...$$`，算法/代码用 fenced 代码块并标注语言，定义贴原文。
   - OCR 缺损但属于本学科**公知标准定义/数值**（如封锁协议级别、协议默认端口号、排序复杂度与稳定性、IEEE754 格式）的内容，**应补自标准教材**，并在该条后加 `> Caveat:` 注明「讲义未详述，补自标准教材」；
   - 仅当内容**非公知且 OCR/答案均无据**时，才保留 `[原文未述]` / `[原文模糊]` / `[?]` 占位，**不得默默填入编造**。
...
4. **考试信号权重（强制覆盖）**：
   - 频次表里出现 **≥2 次**的高频概念，**必须**在正文里有对应的 `**粗体头词**` 条目或独立小节覆盖，不得遗漏；即使讲义 OCR 未详述，也要按规则 1「补自标准教材」补全（典型场景：讲义漏写但真题反复考的 MPLS/SDN、IPSec/SSL/TLS 等）；
   - 被考到 ≥2 次的概念务必详写（公式 + 套路 + Caveat）；lecture 出现但未被考的概念可一句带过（仅定义）。
5. **Worked Examples（强制）**：若上方【本学科考试信号】非空，本区块**必须存在**且至少收录 **2 条**代表题。来源优先级：① exam-signals 第 5 节的本章候选；② 若无本章候选，从本章 OCR 的「例题/专项练习」里选典型题改造（题干精简、解法 3-8 步，无书内答案的标 `[PROPOSED — verify]`）；③ 仅当整章 OCR 完全无任何例题/习题时，才允许只收录 1 条或省略。
...
8. **Worked Example 章节归属过滤（关键，防跨章污染）**：当前正在写的是「{chapter_label}」{chapter_label_hint}。exam-signals 第 5 节的 Worked Example 候选**只允许**选取来源标注为本章的题目（标注形如「{chapter_label}-单选-1」「{chapter_label}-多选-4」「本章-…」等）；**严禁**把来源标注为其它章节（{other_chapter_hint}）的题目塞入本章 cheatsheet。若本章在 exam-signals 无候选，按规则 5 的②回退到本章 OCR 例题自造 WE；**不要凑数塞其它章节的题**。
